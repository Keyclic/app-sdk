...
,
  ...
  return Configuration(
        ...
        memberType: ConfigurationMemberType.fromJson(json[r'memberType']),
                  name: json[r'name'],
        operationType: ConfigurationOperationType.fromJson(json[r'operationType']),
  ...
  static List<Configuration> listFromJson(Iterable? json) {
    ...
    return json.fold(<Configuration>[], (List<Configuration> previousValue, element) {
  ...
  static Map<String, Configuration> mapFromJson(Map<String, dynamic>? json) {
    ...
    return json.entries.fold(<String, Configuration>{}, (Map<String, Configuration> previousValue, element) {
  ...
  static Map<String, List<Configuration>> mapListFromJson(Map<String, dynamic>? json) {
    ...
    return json.map((key, value) {
      return MapEntry<String, List<Configuration>>(key, Configuration.listFromJson(value));
    });
  }

  @override
  String toString() => 'Configuration[links=$links, createdAt=$createdAt, description=$description, id=$id, memberType=$memberType, name=$name, operationType=$operationType, placeType=$placeType, reportType=$reportType, type=$type, updatedAt=$updatedAt]';
  ...
  Map<String, dynamic> toJson([Iterable<String>? keys]) {
    ...
    contains(r'id')
        ...
        r'id':
          id,
    if (keys == null || keys.
    any((key) => RegExp(r'^memberType\.').hasMatch(key))
    )
        r'memberType':
            memberType?.toJson(keys?.fold<List<String>>(<String>[], (List<String> previousValue, String element) {
        ...
        r'name':
          name,
    if (keys == null || keys.
    any((key) => RegExp(r'^operationType\.').hasMatch(key))
    )
        r'operationType':
            operationType?.toJson(keys?.fold<List<String>>(<String>[], (List<String> previousValue, String element) {
    ...
    if (keys == null || keys.
    any((key) => RegExp(r'^placeType\.').hasMatch(key))
    )
        r'placeType':
            placeType?.toJson(keys?.fold<List<String>>(<String>[], (List<String> previousValue, String element) {
    ...
    if (keys == null || keys.
    any((key) => RegExp(r'^reportType\.').hasMatch(key))
    )
        r'reportType':
            reportType?.toJson(keys?.fold<List<String>>(<String>[], (List<String> previousValue, String element) {
