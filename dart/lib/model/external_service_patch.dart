...
,
  ...
  static List<ExternalServicePatch> listFromJson(Iterable? json) {
    ...
    return json.fold(<ExternalServicePatch>[], (List<ExternalServicePatch> previousValue, element) {
      final ExternalServicePatch? object = ExternalServicePatch.fromJson(element);
  ...
  static Map<String, ExternalServicePatch> mapFromJson(Map<String, dynamic>? json) {
    if (json == null) {
      return <String, ExternalServicePatch>{};
    }

    return json.entries.fold(<String, ExternalServicePatch>{}, (Map<String, ExternalServicePatch> previousValue, element) {
      final ExternalServicePatch? object = ExternalServicePatch.fromJson(element.value);
      if (object is ExternalServicePatch) {
  ...
  static Map<String, List<ExternalServicePatch>> mapListFromJson(Map<String, dynamic>? json) {
    if (json == null) {
      return <String, List<ExternalServicePatch>>{};
    }

    return json.map((key, value) {
      return MapEntry<String, List<ExternalServicePatch>>(key, ExternalServicePatch.listFromJson(value));
    });
  }

  @override
  String toString() => 'ExternalServicePatch[address=$address, contactPoint=$contactPoint, description=$description, name=$name]';
