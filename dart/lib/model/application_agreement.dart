...
,
  ...
  static List<ApplicationAgreement> listFromJson(Iterable? json) {
    ...
    return json.fold(<ApplicationAgreement>[], (List<ApplicationAgreement> previousValue, element) {
      final ApplicationAgreement? object = ApplicationAgreement.fromJson(element);
  ...
  static Map<String, ApplicationAgreement> mapFromJson(Map<String, dynamic>? json) {
    if (json == null) {
      return <String, ApplicationAgreement>{};
    }

    return json.entries.fold(<String, ApplicationAgreement>{}, (Map<String, ApplicationAgreement> previousValue, element) {
      final ApplicationAgreement? object = ApplicationAgreement.fromJson(element.value);
      if (object is ApplicationAgreement) {
  ...
  static Map<String, List<ApplicationAgreement>> mapListFromJson(Map<String, dynamic>? json) {
    if (json == null) {
      return <String, List<ApplicationAgreement>>{};
    }

    return json.map((key, value) {
      return MapEntry<String, List<ApplicationAgreement>>(key, ApplicationAgreement.listFromJson(value));
    });
  }

  @override
  String toString() => 'ApplicationAgreement[olderThan=$olderThan, privacyPolicy=$privacyPolicy, termsOfService=$termsOfService]';

  Map<String, dynamic> toJson([Iterable<String>? keys]) {
    return <String, dynamic>{
    if (keys == null || keys.
    any((key) => RegExp(r'^olderThan\.').hasMatch(key))
    )
        r'olderThan':
            olderThan?.toJson(keys?.fold<List<String>>(<String>[], (List<String> previousValue, String element) {
    ...
    if (keys == null || keys.
    any((key) => RegExp(r'^privacyPolicy\.').hasMatch(key))
    )
        r'privacyPolicy':
            privacyPolicy?.toJson(keys?.fold<List<String>>(<String>[], (List<String> previousValue, String element) {
    ...
    if (keys == null || keys.
    any((key) => RegExp(r'^termsOfService\.').hasMatch(key))
    )
        r'termsOfService':
            termsOfService?.toJson(keys?.fold<List<String>>(<String>[], (List<String> previousValue, String element) {
