...
,
  ...
  static List<BusinessActivityLinks> listFromJson(Iterable? json) {
    ...
    return json.fold(<BusinessActivityLinks>[], (List<BusinessActivityLinks> previousValue, element) {
      final BusinessActivityLinks? object = BusinessActivityLinks.fromJson(element);
  ...
  static Map<String, BusinessActivityLinks> mapFromJson(Map<String, dynamic>? json) {
    ...
    return json.entries.fold(<String, BusinessActivityLinks>{}, (Map<String, BusinessActivityLinks> previousValue, element) {
      final BusinessActivityLinks? object = BusinessActivityLinks.fromJson(element.value);
      if (object is BusinessActivityLinks) {
  ...
  static Map<String, List<BusinessActivityLinks>> mapListFromJson(Map<String, dynamic>? json) {
    if (json == null) {
      return <String, List<BusinessActivityLinks>>{};
    }

    return json.map((key, value) {
      return MapEntry<String, List<BusinessActivityLinks>>(key, BusinessActivityLinks.listFromJson(value));
    });
  }

  @override
  String toString() => 'BusinessActivityLinks[image=$image, schema=$schema, self=$self]';
  ...
  Map<String, dynamic> toJson([Iterable<String>? keys]) {
    ...
    if (keys == null || keys.
    any((key) => RegExp(r'^schema\.').hasMatch(key))
    )
        r'schema':
            schema?.toJson(keys?.fold<List<String>>(<String>[], (List<String> previousValue, String element) {
