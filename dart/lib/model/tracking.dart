...
,
  ...
  bool operator ==(Object other) {
    ...
    return other is Tracking 
          && DeepCollectionEquality.unordered().equals(checkpoints, other.checkpoints)
  ...
  static Map<String, Tracking> mapFromJson(Map<String, dynamic>? json) {
    ...
    return json.entries.fold(<String, Tracking>{}, (Map<String, Tracking> previousValue, element) {
  ...
  static Map<String, List<Tracking>> mapListFromJson(Map<String, dynamic>? json) {
    if (json == null) {
      return <String, List<Tracking>>{};
    }

    return json.map((key, value) {
      return MapEntry<String, List<Tracking>>(key, Tracking.listFromJson(value));
    });
  }

  @override
  String toString() => 'Tracking[checkpoints=$checkpoints, progression=$progression, state=$state, time=$time]';

  Map<String, dynamic> toJson([Iterable<String>? keys]) {
    return <String, dynamic>{
    if (keys == null || keys.
    contains(r'checkpoints')
    )
        r'checkpoints':
            checkpoints,
    if (keys == null || keys.
    any((key) => RegExp(r'^progression\.').hasMatch(key))
    )
        r'progression':
            progression?.toJson(keys?.fold<List<String>>(<String>[], (List<String> previousValue, String element) {
