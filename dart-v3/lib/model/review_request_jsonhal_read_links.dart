...
,
  ...
  return ReviewRequestJsonhalReadLinks(
        ...
        itemToReview: GetPlaceCollection200ResponseLinksFirst.fromJson(json[r'itemToReview']),
        reviewer: GetPlaceCollection200ResponseLinksSelf.fromJson(json[r'reviewer']),
  ...
  static List<ReviewRequestJsonhalReadLinks> listFromJson(Iterable? json) {
    ...
    return json.fold(<ReviewRequestJsonhalReadLinks>[], (List<ReviewRequestJsonhalReadLinks> previousValue, element) {
      final ReviewRequestJsonhalReadLinks? object = ReviewRequestJsonhalReadLinks.fromJson(element);
  ...
  static Map<String, ReviewRequestJsonhalReadLinks> mapFromJson(Map<String, dynamic>? json) {
    if (json == null) {
      return <String, ReviewRequestJsonhalReadLinks>{};
    }

    return json.entries.fold(<String, ReviewRequestJsonhalReadLinks>{}, (Map<String, ReviewRequestJsonhalReadLinks> previousValue, element) {
      final ReviewRequestJsonhalReadLinks? object = ReviewRequestJsonhalReadLinks.fromJson(element.value);
      if (object is ReviewRequestJsonhalReadLinks) {
  ...
  static Map<String, List<ReviewRequestJsonhalReadLinks>> mapListFromJson(Map<String, dynamic>? json) {
    if (json == null) {
      return <String, List<ReviewRequestJsonhalReadLinks>>{};
    }

    return json.map((key, value) {
      return MapEntry<String, List<ReviewRequestJsonhalReadLinks>>(key, ReviewRequestJsonhalReadLinks.listFromJson(value));
    });
  }

  @override
  String toString() => 'ReviewRequestJsonhalReadLinks[self=$self, itemToReview=$itemToReview, reviewer=$reviewer, organization=$organization]';
  ...
  Map<String, dynamic> toJson([Iterable<String>? keys]) {
    ...
    if (keys == null || keys.
    any((key) => RegExp(r'^itemToReview\.').hasMatch(key))
    )
        r'itemToReview':
            itemToReview?.toJson(keys?.fold<List<String>>(<String>[], (List<String> previousValue, String element) {
    ...
    if (keys == null || keys.
    any((key) => RegExp(r'^reviewer\.').hasMatch(key))
    )
        r'reviewer':
            reviewer?.toJson(keys?.fold<List<String>>(<String>[], (List<String> previousValue, String element) {
