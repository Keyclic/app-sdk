...
,
  ...
  static List<PlaceTypeWorkflow> listFromJson(Iterable? json) {
    if (json == null) {
      return <PlaceTypeWorkflow>[];
    }

    return json.fold(<PlaceTypeWorkflow>[], (List<PlaceTypeWorkflow> previousValue, element) {
  ...
  static Map<String, PlaceTypeWorkflow> mapFromJson(Map<String, dynamic>? json) {
    ...
    return json.entries.fold(<String, PlaceTypeWorkflow>{}, (Map<String, PlaceTypeWorkflow> previousValue, element) {
      final PlaceTypeWorkflow? object = PlaceTypeWorkflow.fromJson(element.value);
      if (object is PlaceTypeWorkflow) {
  ...
  static Map<String, List<PlaceTypeWorkflow>> mapListFromJson(Map<String, dynamic>? json) {
    if (json == null) {
      return <String, List<PlaceTypeWorkflow>>{};
    }

    return json.map((key, value) {
      return MapEntry<String, List<PlaceTypeWorkflow>>(key, PlaceTypeWorkflow.listFromJson(value));
    });
  }

  @override
  String toString() => 'PlaceTypeWorkflow[createdAt=$createdAt, description=$description, end=$end, id=$id, name=$name, start=$start, states=$states, transitions=$transitions, type=$type, updatedAt=$updatedAt]';
