...
,
  ...
  static List<ReportTypeWorkflow> listFromJson(Iterable? json) {
    if (json == null) {
      return <ReportTypeWorkflow>[];
    }

    return json.fold(<ReportTypeWorkflow>[], (List<ReportTypeWorkflow> previousValue, element) {
  ...
  static Map<String, ReportTypeWorkflow> mapFromJson(Map<String, dynamic>? json) {
    if (json == null) {
      return <String, ReportTypeWorkflow>{};
    }

    return json.entries.fold(<String, ReportTypeWorkflow>{}, (Map<String, ReportTypeWorkflow> previousValue, element) {
      final ReportTypeWorkflow? object = ReportTypeWorkflow.fromJson(element.value);
      if (object is ReportTypeWorkflow) {
  ...
  static Map<String, List<ReportTypeWorkflow>> mapListFromJson(Map<String, dynamic>? json) {
    if (json == null) {
      return <String, List<ReportTypeWorkflow>>{};
    }

    return json.map((key, value) {
      return MapEntry<String, List<ReportTypeWorkflow>>(key, ReportTypeWorkflow.listFromJson(value));
    });
  }

  @override
  String toString() => 'ReportTypeWorkflow[createdAt=$createdAt, description=$description, end=$end, id=$id, name=$name, start=$start, states=$states, transitions=$transitions, type=$type, updatedAt=$updatedAt]';
