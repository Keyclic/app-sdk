...
,
  ...
  return OrganizationLinks(
        ...
        businessActivity: OrganizationLinksBusinessActivity.fromJson(json[r'businessActivity']),
        configuration: OrganizationLinksConfiguration.fromJson(json[r'configuration']),
  ...
  static List<OrganizationLinks> listFromJson(Iterable? json) {
    if (json == null) {
      return <OrganizationLinks>[];
    }

    return json.fold(<OrganizationLinks>[], (List<OrganizationLinks> previousValue, element) {
  ...
  static Map<String, OrganizationLinks> mapFromJson(Map<String, dynamic>? json) {
    ...
    return json.entries.fold(<String, OrganizationLinks>{}, (Map<String, OrganizationLinks> previousValue, element) {
      final OrganizationLinks? object = OrganizationLinks.fromJson(element.value);
      if (object is OrganizationLinks) {
  ...
  static Map<String, List<OrganizationLinks>> mapListFromJson(Map<String, dynamic>? json) {
    if (json == null) {
      return <String, List<OrganizationLinks>>{};
    }

    return json.map((key, value) {
      return MapEntry<String, List<OrganizationLinks>>(key, OrganizationLinks.listFromJson(value));
    });
  }

  @override
  String toString() => 'OrganizationLinks[application=$application, businessActivity=$businessActivity, configuration=$configuration, dispatcher=$dispatcher, logo=$logo, self=$self]';

  Map<String, dynamic> toJson([Iterable<String>? keys]) {
    return <String, dynamic>{
    if (keys == null || keys.
    any((key) => RegExp(r'^application\.').hasMatch(key))
    )
        r'application':
            application?.toJson(keys?.fold<List<String>>(<String>[], (List<String> previousValue, String element) {
    ...
    if (keys == null || keys.
    any((key) => RegExp(r'^businessActivity\.').hasMatch(key))
    )
        r'businessActivity':
            businessActivity?.toJson(keys?.fold<List<String>>(<String>[], (List<String> previousValue, String element) {
              if (element.contains(RegExp(r'^businessActivity\.'))) {
                previousValue.add(element.split(RegExp(r'^businessActivity\.')).last);
              }

              return previousValue;
            })),
    if (keys == null || keys.
    any((key) => RegExp(r'^configuration\.').hasMatch(key))
    )
        r'configuration':
            configuration?.toJson(keys?.fold<List<String>>(<String>[], (List<String> previousValue, String element) {
    ...
    if (keys == null || keys.
    any((key) => RegExp(r'^dispatcher\.').hasMatch(key))
    )
        r'dispatcher':
            dispatcher?.toJson(keys?.fold<List<String>>(<String>[], (List<String> previousValue, String element) {
    ...
    if (keys == null || keys.
    any((key) => RegExp(r'^logo\.').hasMatch(key))
    )
        r'logo':
            logo?.toJson(keys?.fold<List<String>>(<String>[], (List<String> previousValue, String element) {
