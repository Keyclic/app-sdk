...
,
    ...
    this.state
       = const ContractCreateContractCommandWriteStateEnum._('DRAFT')
    ,
...
,
  ...
  static List<ContractCreateContractCommandWrite> listFromJson(Iterable? json) {
    ...
    return json.fold(<ContractCreateContractCommandWrite>[], (List<ContractCreateContractCommandWrite> previousValue, element) {
      final ContractCreateContractCommandWrite? object = ContractCreateContractCommandWrite.fromJson(element);
  ...
  static Map<String, ContractCreateContractCommandWrite> mapFromJson(Map<String, dynamic>? json) {
    if (json == null) {
      return <String, ContractCreateContractCommandWrite>{};
    }

    return json.entries.fold(<String, ContractCreateContractCommandWrite>{}, (Map<String, ContractCreateContractCommandWrite> previousValue, element) {
      final ContractCreateContractCommandWrite? object = ContractCreateContractCommandWrite.fromJson(element.value);
      if (object is ContractCreateContractCommandWrite) {
  ...
  static Map<String, List<ContractCreateContractCommandWrite>> mapListFromJson(Map<String, dynamic>? json) {
    if (json == null) {
      return <String, List<ContractCreateContractCommandWrite>>{};
    }

    return json.map((key, value) {
      return MapEntry<String, List<ContractCreateContractCommandWrite>>(key, ContractCreateContractCommandWrite.listFromJson(value));
    });
  }

  @override
  String toString() => 'ContractCreateContractCommandWrite[billing=$billing, description=$description, duration=$duration, effectiveDate=$effectiveDate, name=$name, number=$number, onCall=$onCall, places=$places, provider=$provider, renewal=$renewal, signedAt=$signedAt, state=$state, terminationDate=$terminationDate, terminationReason=$terminationReason, type=$type]';

  Map<String, dynamic> toJson([Iterable<String>? keys]) {
    return <String, dynamic>{
    if (keys == null || keys.
    any((key) => RegExp(r'^billing\.').hasMatch(key))
    )
        r'billing':
            billing?.toJson(keys?.fold<List<String>>(<String>[], (List<String> previousValue, String element) {
    ...
    contains(r'places')
        ...
        r'provider':
          provider,
    if (keys == null || keys.
    any((key) => RegExp(r'^renewal\.').hasMatch(key))
    )
        r'renewal':
            renewal?.toJson(keys?.fold<List<String>>(<String>[], (List<String> previousValue, String element) {
...
class ContractCreateContractCommandWriteStateEnum {
  ...
  static const ACTIVE = ContractCreateContractCommandWriteStateEnum._(r'ACTIVE');
  static const DRAFT = ContractCreateContractCommandWriteStateEnum._(r'DRAFT');
  static const SUSPENDED = ContractCreateContractCommandWriteStateEnum._(r'SUSPENDED');
  ...
  static ContractCreateContractCommandWriteStateEnum? fromJson(dynamic value) =>
    ContractCreateContractCommandWriteStateEnumTypeTransformer().decode(value);

  static List<ContractCreateContractCommandWriteStateEnum> listFromJson(List<dynamic> json) {
    return json.map((value) {
      return ContractCreateContractCommandWriteStateEnum.fromJson(value);
    }).whereType<ContractCreateContractCommandWriteStateEnum>().toList();
  }
}
...
class ContractCreateContractCommandWriteStateEnumTypeTransformer {
  ...
  factory ContractCreateContractCommandWriteStateEnumTypeTransformer() => _instance ??= ContractCreateContractCommandWriteStateEnumTypeTransformer._();
  ...
  ContractCreateContractCommandWriteStateEnum? decode(dynamic data, {bool allowNull = true}) {
    switch (data) {
      case r'ACTIVE': return ContractCreateContractCommandWriteStateEnum.ACTIVE;
      case r'DRAFT': return ContractCreateContractCommandWriteStateEnum.DRAFT;
      case r'SUSPENDED': return ContractCreateContractCommandWriteStateEnum.SUSPENDED;
      default:
        if (allowNull == false) {
