...
,
  ...
  static List<TicketEditTicketCommandWrite> listFromJson(Iterable? json) {
    ...
    return json.fold(<TicketEditTicketCommandWrite>[], (List<TicketEditTicketCommandWrite> previousValue, element) {
      final TicketEditTicketCommandWrite? object = TicketEditTicketCommandWrite.fromJson(element);
  ...
  static Map<String, TicketEditTicketCommandWrite> mapFromJson(Map<String, dynamic>? json) {
    if (json == null) {
      return <String, TicketEditTicketCommandWrite>{};
    }

    return json.entries.fold(<String, TicketEditTicketCommandWrite>{}, (Map<String, TicketEditTicketCommandWrite> previousValue, element) {
      final TicketEditTicketCommandWrite? object = TicketEditTicketCommandWrite.fromJson(element.value);
      if (object is TicketEditTicketCommandWrite) {
  ...
  static Map<String, List<TicketEditTicketCommandWrite>> mapListFromJson(Map<String, dynamic>? json) {
    if (json == null) {
      return <String, List<TicketEditTicketCommandWrite>>{};
    }

    return json.map((key, value) {
      return MapEntry<String, List<TicketEditTicketCommandWrite>>(key, TicketEditTicketCommandWrite.listFromJson(value));
    });
  }

  @override
  String toString() => 'TicketEditTicketCommandWrite[archived=$archived, category=$category, completed=$completed, description=$description, dueBy=$dueBy, identificationNumber=$identificationNumber, name=$name, priority=$priority, scheduledAt=$scheduledAt, tags=$tags]';
