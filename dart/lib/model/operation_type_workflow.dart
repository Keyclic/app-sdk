...
,
  ...
  static List<OperationTypeWorkflow> listFromJson(Iterable? json) {
    ...
    return json.fold(<OperationTypeWorkflow>[], (List<OperationTypeWorkflow> previousValue, element) {
      final OperationTypeWorkflow? object = OperationTypeWorkflow.fromJson(element);
  ...
  static Map<String, OperationTypeWorkflow> mapFromJson(Map<String, dynamic>? json) {
    if (json == null) {
      return <String, OperationTypeWorkflow>{};
    }

    return json.entries.fold(<String, OperationTypeWorkflow>{}, (Map<String, OperationTypeWorkflow> previousValue, element) {
      final OperationTypeWorkflow? object = OperationTypeWorkflow.fromJson(element.value);
      if (object is OperationTypeWorkflow) {
  ...
  static Map<String, List<OperationTypeWorkflow>> mapListFromJson(Map<String, dynamic>? json) {
    if (json == null) {
      return <String, List<OperationTypeWorkflow>>{};
    }

    return json.map((key, value) {
      return MapEntry<String, List<OperationTypeWorkflow>>(key, OperationTypeWorkflow.listFromJson(value));
    });
  }

  @override
  String toString() => 'OperationTypeWorkflow[createdAt=$createdAt, description=$description, end=$end, id=$id, name=$name, start=$start, states=$states, transitions=$transitions, type=$type, updatedAt=$updatedAt]';
  ...
  Map<String, dynamic> toJson([Iterable<String>? keys]) {
    ...
    contains(r'description')
        ...
        r'description':
          description,
    if (keys == null || keys.
    any((key) => RegExp(r'^end\.').hasMatch(key))
    )
        r'end':
            end?.toJson(keys?.fold<List<String>>(<String>[], (List<String> previousValue, String element) {
    ...
    contains(r'name')
        ...
        r'name':
          name,
    if (keys == null || keys.
    any((key) => RegExp(r'^start\.').hasMatch(key))
    )
        r'start':
            start?.toJson(keys?.fold<List<String>>(<String>[], (List<String> previousValue, String element) {
