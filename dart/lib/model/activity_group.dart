...
,
  ...
  bool operator ==(Object other) {
    ...
    return other is ActivityGroup 
          && DeepCollectionEquality.unordered().equals(activities, other.activities)
  ...
  static List<ActivityGroup> listFromJson(Iterable? json) {
    ...
    return json.fold(<ActivityGroup>[], (List<ActivityGroup> previousValue, element) {
  ...
  static Map<String, ActivityGroup> mapFromJson(Map<String, dynamic>? json) {
    ...
    return json.entries.fold(<String, ActivityGroup>{}, (Map<String, ActivityGroup> previousValue, element) {
  ...
  static Map<String, List<ActivityGroup>> mapListFromJson(Map<String, dynamic>? json) {
    if (json == null) {
      return <String, List<ActivityGroup>>{};
    }

    return json.map((key, value) {
      return MapEntry<String, List<ActivityGroup>>(key, ActivityGroup.listFromJson(value));
    });
  }

  @override
  String toString() => 'ActivityGroup[activities=$activities, activityCount=$activityCount, actorCount=$actorCount, createdAt=$createdAt, group=$group, id=$id, updatedAt=$updatedAt, verb=$verb, isRead=$isRead, isSeen=$isSeen]';

  Map<String, dynamic> toJson([Iterable<String>? keys]) {
    return <String, dynamic>{
    if (keys == null || keys.
    contains(r'activities')
    )
        r'activities':
            activities,
    if (keys == null || keys.
    contains(r'activityCount')
    )
        r'activity_count':
          activityCount,
    if (keys == null || keys.
    contains(r'actorCount')
    )
        r'actor_count':
          actorCount,
    if (keys == null || keys.
