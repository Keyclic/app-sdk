...
,
  ...
  return TicketJsonhalReadEmbedded(
        assignments: 
          AssignmentJsonhalRead.listFromJson(json[r'assignments']),
        category: CategoryJsonhalRead.fromJson(json[r'category']),
        equipments: 
          EquipmentJsonhalRead.listFromJson(json[r'equipments']),
  ...
  bool operator ==(Object other) {
    ...
    return other is TicketJsonhalReadEmbedded 
          && DeepCollectionEquality.unordered().equals(assignments, other.assignments)
  ...
  static List<TicketJsonhalReadEmbedded> listFromJson(Iterable? json) {
    ...
    return json.fold(<TicketJsonhalReadEmbedded>[], (List<TicketJsonhalReadEmbedded> previousValue, element) {
      final TicketJsonhalReadEmbedded? object = TicketJsonhalReadEmbedded.fromJson(element);
  ...
  static Map<String, TicketJsonhalReadEmbedded> mapFromJson(Map<String, dynamic>? json) {
    if (json == null) {
      return <String, TicketJsonhalReadEmbedded>{};
    }

    return json.entries.fold(<String, TicketJsonhalReadEmbedded>{}, (Map<String, TicketJsonhalReadEmbedded> previousValue, element) {
      final TicketJsonhalReadEmbedded? object = TicketJsonhalReadEmbedded.fromJson(element.value);
      if (object is TicketJsonhalReadEmbedded) {
  ...
  static Map<String, List<TicketJsonhalReadEmbedded>> mapListFromJson(Map<String, dynamic>? json) {
    if (json == null) {
      return <String, List<TicketJsonhalReadEmbedded>>{};
    }

    return json.map((key, value) {
      return MapEntry<String, List<TicketJsonhalReadEmbedded>>(key, TicketJsonhalReadEmbedded.listFromJson(value));
    });
  }

  @override
  String toString() => 'TicketJsonhalReadEmbedded[assignments=$assignments, category=$category, equipments=$equipments, phase=$phase, place=$place, priority=$priority, feedback=$feedback, children=$children, state=$state]';

  Map<String, dynamic> toJson([Iterable<String>? keys]) {
    return <String, dynamic>{
    if (keys == null || keys.
    contains(r'assignments')
    )
        r'assignments':
            assignments,
    if (keys == null || keys.
    any((key) => RegExp(r'^category\.').hasMatch(key))
    )
        r'category':
            category?.toJson(keys?.fold<List<String>>(<String>[], (List<String> previousValue, String element) {
    ...
    contains(r'equipments')
        ...
        r'equipments':
            equipments,
    if (keys == null || keys.
    any((key) => RegExp(r'^phase\.').hasMatch(key))
    )
        r'phase':
            phase?.toJson(keys?.fold<List<String>>(<String>[], (List<String> previousValue, String element) {
