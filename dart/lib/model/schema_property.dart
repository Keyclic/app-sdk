...
,
  ...
  static SchemaProperty? fromJson(Object? json) {
  ...
  static List<SchemaProperty> listFromJson(Iterable? json) {
    if (json == null) {
      return <SchemaProperty>[];
    }

    return json.fold(<SchemaProperty>[], (List<SchemaProperty> previousValue, element) {
  ...
  static Map<String, SchemaProperty> mapFromJson(Map<String, dynamic>? json) {
    ...
    return json.entries.fold(<String, SchemaProperty>{}, (Map<String, SchemaProperty> previousValue, element) {
  ...
  static Map<String, List<SchemaProperty>> mapListFromJson(Map<String, dynamic>? json) {
    if (json == null) {
      return <String, List<SchemaProperty>>{};
    }

    return json.map((key, value) {
      return MapEntry<String, List<SchemaProperty>>(key, SchemaProperty.listFromJson(value));
    });
  }

  @override
  String toString() => 'SchemaProperty[conditions=$conditions, default_=$default_, description=$description, enum_=$enum_, format=$format, id=$id, items=$items, maxItems=$maxItems, minItems=$minItems, propertyOrder=$propertyOrder, title=$title, type=$type]';

  Map<String, dynamic> toJson([Iterable<String>? keys]) {
    return <String, dynamic>{
    if (keys == null || keys.
    any((key) => RegExp(r'^conditions\.').hasMatch(key))
    )
        r'conditions':
            conditions?.toJson(keys?.fold<List<String>>(<String>[], (List<String> previousValue, String element) {
    ...
    contains(r'id')
        ...
        r'id':
          id,
    if (keys == null || keys.
    any((key) => RegExp(r'^items\.').hasMatch(key))
    )
        r'items':
            items?.toJson(keys?.fold<List<String>>(<String>[], (List<String> previousValue, String element) {
