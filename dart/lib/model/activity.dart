...
,
  ...
  static Map<String, Activity> mapFromJson(Map<String, dynamic>? json) {
    ...
    return json.entries.fold(<String, Activity>{}, (Map<String, Activity> previousValue, element) {
  ...
  static Map<String, List<Activity>> mapListFromJson(Map<String, dynamic>? json) {
    ...
    return json.map((key, value) {
      return MapEntry<String, List<Activity>>(key, Activity.listFromJson(value));
    });
  }

  @override
  String toString() => 'Activity[actor=$actor, message=$message, object=$object, origin=$origin, subject=$subject, time=$time, title=$title, verb=$verb]';
  ...
  Map<String, dynamic> toJson([Iterable<String>? keys]) {
    ...
    contains(r'origin')
        ...
        r'origin':
          origin,
    if (keys == null || keys.
    any((key) => RegExp(r'^subject\.').hasMatch(key))
    )
        r'subject':
            subject?.toJson(keys?.fold<List<String>>(<String>[], (List<String> previousValue, String element) {
