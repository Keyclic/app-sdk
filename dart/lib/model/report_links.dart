...
,
  ...
  return ReportLinks(
        ...
        equipments: 
          ReportLinksEquipments.listFromJson(json[r'equipments']),
  ...
  static List<ReportLinks> listFromJson(Iterable? json) {
    ...
    return json.fold(<ReportLinks>[], (List<ReportLinks> previousValue, element) {
  ...
  static Map<String, ReportLinks> mapFromJson(Map<String, dynamic>? json) {
    ...
    return json.entries.fold(<String, ReportLinks>{}, (Map<String, ReportLinks> previousValue, element) {
  ...
  static Map<String, List<ReportLinks>> mapListFromJson(Map<String, dynamic>? json) {
    if (json == null) {
      return <String, List<ReportLinks>>{};
    }

    return json.map((key, value) {
      return MapEntry<String, List<ReportLinks>>(key, ReportLinks.listFromJson(value));
    });
  }

  @override
  String toString() => 'ReportLinks[asset=$asset, category=$category, children=$children, createdBy=$createdBy, equipments=$equipments, feedback=$feedback, operations=$operations, organization=$organization, place=$place, self=$self, tracking=$tracking, workflow=$workflow]';

  Map<String, dynamic> toJson([Iterable<String>? keys]) {
    return <String, dynamic>{
    if (keys == null || keys.
    any((key) => RegExp(r'^asset\.').hasMatch(key))
    )
        r'asset':
            asset?.toJson(keys?.fold<List<String>>(<String>[], (List<String> previousValue, String element) {
    ...
    if (keys == null || keys.
    any((key) => RegExp(r'^children\.').hasMatch(key))
    )
        r'children':
            children?.toJson(keys?.fold<List<String>>(<String>[], (List<String> previousValue, String element) {
    ...
    if (keys == null || keys.
    any((key) => RegExp(r'^operations\.').hasMatch(key))
    )
        r'operations':
            operations?.toJson(keys?.fold<List<String>>(<String>[], (List<String> previousValue, String element) {
    ...
    if (keys == null || keys.
    any((key) => RegExp(r'^tracking\.').hasMatch(key))
    )
        r'tracking':
            tracking?.toJson(keys?.fold<List<String>>(<String>[], (List<String> previousValue, String element) {
