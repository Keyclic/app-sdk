...
,
  ...
  static List<TrackingCheckpoint> listFromJson(Iterable? json) {
    ...
    return json.fold(<TrackingCheckpoint>[], (List<TrackingCheckpoint> previousValue, element) {
  ...
  static Map<String, TrackingCheckpoint> mapFromJson(Map<String, dynamic>? json) {
    if (json == null) {
      return <String, TrackingCheckpoint>{};
    }

    return json.entries.fold(<String, TrackingCheckpoint>{}, (Map<String, TrackingCheckpoint> previousValue, element) {
      final TrackingCheckpoint? object = TrackingCheckpoint.fromJson(element.value);
      if (object is TrackingCheckpoint) {
  ...
  static Map<String, List<TrackingCheckpoint>> mapListFromJson(Map<String, dynamic>? json) {
    if (json == null) {
      return <String, List<TrackingCheckpoint>>{};
    }

    return json.map((key, value) {
      return MapEntry<String, List<TrackingCheckpoint>>(key, TrackingCheckpoint.listFromJson(value));
    });
  }

  @override
  String toString() => 'TrackingCheckpoint[createdAt=$createdAt, data=$data, state=$state, links=$links, embedded=$embedded]';
  ...
  Map<String, dynamic> toJson([Iterable<String>? keys]) {
    ...
    if (keys == null || keys.
    any((key) => RegExp(r'^data\.').hasMatch(key))
    )
        r'data':
            data?.toJson(keys?.fold<List<String>>(<String>[], (List<String> previousValue, String element) {
