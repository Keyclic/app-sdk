...
,
  ...
  static Map<String, Document> mapFromJson(Map<String, dynamic>? json) {
    ...
    return json.entries.fold(<String, Document>{}, (Map<String, Document> previousValue, element) {
  ...
  static Map<String, List<Document>> mapListFromJson(Map<String, dynamic>? json) {
    if (json == null) {
      return <String, List<Document>>{};
    }

    return json.map((key, value) {
      return MapEntry<String, List<Document>>(key, Document.listFromJson(value));
    });
  }

  @override
  String toString() => 'Document[embedded=$embedded, links=$links, body=$body, createdAt=$createdAt, file=$file, id=$id, permission=$permission, state=$state, tags=$tags, text=$text, type=$type, updatedAt=$updatedAt]';
...
class DocumentStateEnum {
  ...
  static List<DocumentStateEnum> listFromJson(List<dynamic> json) {
    ...
    }).whereType<DocumentStateEnum>().toList();
  }
}
...
class DocumentStateEnumTypeTransformer {
  ...
  factory DocumentStateEnumTypeTransformer() => _instance ??= DocumentStateEnumTypeTransformer._();
  ...
  DocumentStateEnum? decode(dynamic data, {bool allowNull = true}) {
    switch (data) {
      case r'DRAFT': return DocumentStateEnum.DRAFT;
      case r'PENDING_REVIEW': return DocumentStateEnum.PENDING_REVIEW;
      case r'PUBLISHED': return DocumentStateEnum.PUBLISHED;
      case r'PUBLISHING': return DocumentStateEnum.PUBLISHING;
      case r'PUBLISHING_FAILED': return DocumentStateEnum.PUBLISHING_FAILED;
      default:
        if (allowNull == false) {
