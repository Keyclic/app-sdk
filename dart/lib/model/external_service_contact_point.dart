...
,
  ...
  static List<ExternalServiceContactPoint> listFromJson(Iterable? json) {
    ...
    return json.fold(<ExternalServiceContactPoint>[], (List<ExternalServiceContactPoint> previousValue, element) {
      final ExternalServiceContactPoint? object = ExternalServiceContactPoint.fromJson(element);
  ...
  static Map<String, ExternalServiceContactPoint> mapFromJson(Map<String, dynamic>? json) {
    if (json == null) {
      return <String, ExternalServiceContactPoint>{};
    }

    return json.entries.fold(<String, ExternalServiceContactPoint>{}, (Map<String, ExternalServiceContactPoint> previousValue, element) {
      final ExternalServiceContactPoint? object = ExternalServiceContactPoint.fromJson(element.value);
      if (object is ExternalServiceContactPoint) {
  ...
  static Map<String, List<ExternalServiceContactPoint>> mapListFromJson(Map<String, dynamic>? json) {
    if (json == null) {
      return <String, List<ExternalServiceContactPoint>>{};
    }

    return json.map((key, value) {
      return MapEntry<String, List<ExternalServiceContactPoint>>(key, ExternalServiceContactPoint.listFromJson(value));
    });
  }

  @override
  String toString() => 'ExternalServiceContactPoint[description=$description, email=$email, familyName=$familyName, faxNumber=$faxNumber, givenName=$givenName, isOpen=$isOpen, name=$name, telephone=$telephone]';
