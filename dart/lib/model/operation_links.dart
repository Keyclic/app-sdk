...
,
  ...
  static List<OperationLinks> listFromJson(Iterable? json) {
    ...
    return json.fold(<OperationLinks>[], (List<OperationLinks> previousValue, element) {
  ...
  static Map<String, OperationLinks> mapFromJson(Map<String, dynamic>? json) {
    ...
    return json.entries.fold(<String, OperationLinks>{}, (Map<String, OperationLinks> previousValue, element) {
  ...
  static Map<String, List<OperationLinks>> mapListFromJson(Map<String, dynamic>? json) {
    if (json == null) {
      return <String, List<OperationLinks>>{};
    }

    return json.map((key, value) {
      return MapEntry<String, List<OperationLinks>>(key, OperationLinks.listFromJson(value));
    });
  }

  @override
  String toString() => 'OperationLinks[assignments=$assignments, feedback=$feedback, images=$images, operator_=$operator_, organization=$organization, report=$report, self=$self, tracking=$tracking]';

  Map<String, dynamic> toJson([Iterable<String>? keys]) {
    return <String, dynamic>{
    if (keys == null || keys.
    any((key) => RegExp(r'^assignments\.').hasMatch(key))
    )
        r'assignments':
            assignments?.toJson(keys?.fold<List<String>>(<String>[], (List<String> previousValue, String element) {
    ...
    contains(r'images')
        ...
        r'images':
            images,
    if (keys == null || keys.
    any((key) => RegExp(r'^operator_\.').hasMatch(key))
    )
        r'operator':
            operator_?.toJson(keys?.fold<List<String>>(<String>[], (List<String> previousValue, String element) {
