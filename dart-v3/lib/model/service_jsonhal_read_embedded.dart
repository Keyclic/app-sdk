...
,
  ...
  static List<ServiceJsonhalReadEmbedded> listFromJson(Iterable? json) {
    ...
    return json.fold(<ServiceJsonhalReadEmbedded>[], (List<ServiceJsonhalReadEmbedded> previousValue, element) {
      final ServiceJsonhalReadEmbedded? object = ServiceJsonhalReadEmbedded.fromJson(element);
  ...
  static Map<String, ServiceJsonhalReadEmbedded> mapFromJson(Map<String, dynamic>? json) {
    if (json == null) {
      return <String, ServiceJsonhalReadEmbedded>{};
    }

    return json.entries.fold(<String, ServiceJsonhalReadEmbedded>{}, (Map<String, ServiceJsonhalReadEmbedded> previousValue, element) {
      final ServiceJsonhalReadEmbedded? object = ServiceJsonhalReadEmbedded.fromJson(element.value);
      if (object is ServiceJsonhalReadEmbedded) {
  ...
  static Map<String, List<ServiceJsonhalReadEmbedded>> mapListFromJson(Map<String, dynamic>? json) {
    if (json == null) {
      return <String, List<ServiceJsonhalReadEmbedded>>{};
    }

    return json.map((key, value) {
      return MapEntry<String, List<ServiceJsonhalReadEmbedded>>(key, ServiceJsonhalReadEmbedded.listFromJson(value));
    });
  }

  @override
  String toString() => 'ServiceJsonhalReadEmbedded[provider=$provider, manager=$manager, organization=$organization]';
