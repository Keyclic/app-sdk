...
,
  ...
  static List<ReportEmbedded> listFromJson(Iterable? json) {
    ...
    return json.fold(<ReportEmbedded>[], (List<ReportEmbedded> previousValue, element) {
  ...
  static Map<String, ReportEmbedded> mapFromJson(Map<String, dynamic>? json) {
    ...
    return json.entries.fold(<String, ReportEmbedded>{}, (Map<String, ReportEmbedded> previousValue, element) {
  ...
  static Map<String, List<ReportEmbedded>> mapListFromJson(Map<String, dynamic>? json) {
    if (json == null) {
      return <String, List<ReportEmbedded>>{};
    }

    return json.map((key, value) {
      return MapEntry<String, List<ReportEmbedded>>(key, ReportEmbedded.listFromJson(value));
    });
  }

  @override
  String toString() => 'ReportEmbedded[assignment=$assignment, category=$category, children=$children, documents=$documents, feedback=$feedback, place=$place, targetGroups=$targetGroups, workflow=$workflow]';

  Map<String, dynamic> toJson([Iterable<String>? keys]) {
    return <String, dynamic>{
    if (keys == null || keys.
    any((key) => RegExp(r'^assignment\.').hasMatch(key))
    )
        r'assignment':
            assignment?.toJson(keys?.fold<List<String>>(<String>[], (List<String> previousValue, String element) {
